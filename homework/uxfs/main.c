             ...
             struct fuse_file_info *info)
{
    ...
    /* TODO */
    int count = 0;
    inode_t *leaf = (inode_t *) info->fh;
    if (!leaf) {
        log_printf("Invalid file handle\n");
        return -EBADF;
    }
    count = inode_read(leaf, offset, buf, size);
    log_printf_my("read count: %d\n",count);
    return count;
}
              ...
              struct fuse_file_info *info)
{
    ...
    count = inode_write(leaf, offset, (void*)buf, size);
    log_printf_my("write count: %d\n",count);
    return count;
}
...
int uxfs_readdir(const char *path, void *buf, fuse_fill_dir_t fill, 
    ...
    /* TODO */
    // 必须填充 "." 和 ".." 目录项
    fill(buf, ".", NULL, offset);      // 当前目录
    fill(buf, "..", NULL, offset);     // 上级目录

    while (dir_entry = dir_cursor_next(&dir_cursor)) {
        if (dir_entry->ino == FAKE_INO)
            continue;
        
         // 填充目录项（名称、inode、文件类型等）
         struct stat st;
         memset(&st, 0, sizeof(st));
         st.st_ino = dir_entry->ino;
        
         log_printf_my("readdir directs: %s\n", dir_entry->name);
         // 调用fill函数填充缓冲区
         if (fill(buf, dir_entry->name, &st, offset)) {
             // 缓冲区已满，需要中断并返回（由FUSE处理分页）
             break;
         }
    }
